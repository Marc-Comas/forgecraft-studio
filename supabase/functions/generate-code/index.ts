...
serve(async (req) => {
  ...
  try {
    ...
    const isDualPhase = requestBody.dualPhase === true; // Default to single-phase; enable explicitly
...
IMPORTANT: Return ONLY the JSON object, no markdown formatting.`;
    ...
    const phase1Response = await fetchWithTimeout('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: {
...
IMPORTANT: Return the complete HTML including the JavaScript enhancements.`;
    ...
    const phase2Response = await fetchWithTimeout('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',

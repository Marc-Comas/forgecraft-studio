...
function injectForgeCSSandJS(html: string): string {
  try {
    ...
    if (!doc.getElementById("forgecss")) {
      ...
      style.id = "forgecss";
      style.innerHTML = `
:root{--bg:#0b0f1c;--card:#12172a;--muted:#a7b0d7;--text:#f3f6ff;--brand:#b8ff00;--accent:#00d4ff;--radius:16px;}
...
const Editor = () => {
  ...
  const loadProject = async (id: string, userId: string) => {
    try {
      ...
      const __enh = injectForgeCSSandJS(data.html_content || '');
      setCode(__enh);
      setCodeHistory([__enh]);
      setCanRevert(false);
    } catch (error: any) {
      toast({
        title: "Error",
...
- Include all CSS in <style> tags, all JS in <script> tags`;
      ...
      addToHistory(code);
      const __enhAI = injectForgeCSSandJS(generatedContent);
      setCode(__enhAI);
      setAiPrompt('');
      // Auto-save the AI-generated code
      await saveCodeToDatabase(__enhAI);
